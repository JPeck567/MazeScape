...
class Key(pygame.sprite.Sprite):
    def __init__(self, pos):
        ...
        self.image = pygame.image.load("C:/Users/Jorge/OneDrive - King Edward VI College, Stourbridge/Computer Science/2. Coursework/Project Work/Mazescape/assets/key.png").convert_alpha()
        self.rect = self.image.get_rect(topleft=pos)
...
class Door(pygame.sprite.Sprite):
    def __init__(self, pos):
        ...
        self.door_closed_image = pygame.image.load("C:/Users/Jorge/OneDrive - King Edward VI College, Stourbridge/Computer Science/2. Coursework/Project Work/Mazescape/assets/door_closed.png").convert_alpha()
        self.door_open_image = pygame.image.load("C:/Users/Jorge/OneDrive - King Edward VI College, Stourbridge/Computer Science/2. Coursework/Project Work/Mazescape/assets/door_open.png").convert_alpha()
...
# All Network_ methods are called from the server.
    ...
    # selection screen, once the id message has been recieved
    ##########################
    ...
    def collisions(self):
        ...
        elif pygame.sprite.collide_rect(self.player, self.enemy) and self.player_group.has(self.player):
            # collision detection will carry on even if player is dead and not blitted, so we check if player is dead
            self.player_group.remove(self.player)
            self.enemy_group.remove(self.enemy)
            connection.Send({"action": "player_kill", "gameid": self.gameid, "player": self.userNum, "own": True})
            self.players_alive -= 1

        elif pygame.sprite.collide_rect(self.player, self.other_enemy) and self.player_group.has(self.player):
            self.player_group.remove(self.player)
            self.enemy_group.remove(self.enemy)
            connection.Send({"action": "player_kill", "gameid": self.gameid, "player": self.userNum, "own": False})
            self.players_alive -= 1
    ...
    ##########################
    def enemy_run(self):
        if len(self.enemy.route) in [0, None]:  # if nowhere to go, assume player is caught and enemy gone. added for robustness
            pass
    ...
    def event_loop(self):
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                if self.state is "game":
                    connection.Send({"action": "manual_disconnect", "game": True, "gameid": self.gameid,
                                     "player": self.userNum})
                elif self.state is "selection":
                    connection.Send({"action": "manual_disconnect", "game": False, "id": self.p_id,
                                     "c_type": self.c_type})
    ...
    def loop(self):
        ...
        if self.state == "game":  # if game is started
            self.game_loop()
            self.ping -= (pygame.time.get_ticks() - pygame.time.get_ticks)
            self.last_tick = pygame.time.get_ticks()
            if self.ping <= 0:
                print("Server not responding or disconnected")
                self.state = None
            connection.Send({"action": "ping", "type": "game", "id": self.p_id})
