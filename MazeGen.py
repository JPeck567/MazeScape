...
class Cell:
    def __init__(self, xpos, ypos, size):
        ...
        self.rectDisp = pygame.Rect(self.pixelpos[0], self.pixelpos[1], size, size)
        self.point_list_disp = {"N": [self.rectDisp.topleft, self.rectDisp.topright],
         "E": [self.rectDisp.topright, self.rectDisp.bottomright],
         "S": [self.rectDisp.bottomright, self.rectDisp.bottomleft],
         "W": [self.rectDisp.topleft, self.rectDisp.bottomleft]}
    ...
    def display(self, screen):
        ...
        for x in range(M.length):
            ...
            for y in range(M.width):
                y_coord += 1
                pygame.draw.rect(screen, WHITE, M.maze[x][y].rectDisp)
                for dire, torf in M.maze[x][y].walls.items():
                    line_coord = M.maze[x][y].point_list_disp[dire]
                    pygame.draw.line(screen, TEAL, line_coord[0], line_coord[1], 2)
                    print("ok")
            y_coord = -1
